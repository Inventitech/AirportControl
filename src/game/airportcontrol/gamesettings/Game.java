...
public class Game extends BasicGame{
	...
	public Game() {
		super("Airport Control by Moritz & Steffen, supported via VoiceChat by Lisa (Seniorsupport Clara and Bjoern)");
	}
	@Override
	...
	public void update(GameContainer container, int delta)
		...
		if(true == GameSetup.doCollisionChecks) {
			if (Collision.checkCollision(airplanes)){
				// TODO (MMB) Add collision effect, end game awful use of particles
				collision();
				for(int i=2;i<6;i++) {
	...
	public void render(GameContainer container, Graphics g) throws SlickException {
		...
		airport.getImage().draw(0,0,container.getWidth(), container.getHeight());
		g.drawString(System.getProperty("user.name") + ", " + airport.getLandedAirplanes(), 0, 25);
		Image img;
		int ax =0;
		int ay =0;
		double an =0;
		for (Airplane curAirplane : airplanes) {
			...
			img.setCenterOfRotation((int)(0.5*img.getWidth()*1), // TODO ADD scale factor instead of 1
					(int)(0.5*img.getHeight()*1));
			img.rotate((float)curAirplane.getAngle());
			Color color = new Color(1f, 1f, 1f, (float)curAirplane.getTransparency()); 
			img.draw(curAirplane.getPosition().x-curAirplane.getImage().getWidth()/2,
					curAirplane.getPosition().y-curAirplane.getImage().getWidth()/2,color);
			
			if (curAirplane.getWayPoints() != null){
				for (int i = 1; i < curAirplane.getWayPoints().size(); i++) {
					g.drawLine(curAirplane.getWayPoints().get(i-1).x, curAirplane.getWayPoints().get(i-1).y, curAirplane.getWayPoints().get(i).x, curAirplane.getWayPoints().get(i).y);
				}
			}
	...
	public static void main(String[] args) {
		try {
			...
			app.setDisplayMode(GameSetup.resWidth, GameSetup.resHeight, GameSetup.fullScreenMode);
